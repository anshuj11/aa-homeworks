...
// Examples:
//
//console.log(minChange([1, 2, 5], 11, (memo = {}))); // => 3, because 5 + 5 + 1 = 11
//console.log(minChange([1, 4, 5], 8, (memo = {}))); // => 2, because 4 + 4 = 8
...
function minChange(coins, amount, memo = {}) {
  ...
  let arr = [];
  coins.forEach(c => {
    if (c <= amount) arr.push(minChange(coins, amount - c, memo) + 1);
  });
  memo[amount] = Math.min(...arr);
  return Math.min(...arr);
}
